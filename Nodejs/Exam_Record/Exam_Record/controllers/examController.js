...
exports.examRecord = async (req, res) => {
  const datacount = 200;
  let totalpage = Math.floor(datacount / process.env.TOTAL_DATA);
  let page;
  ...
  // console.log(result);
  res.render("examTable", { data: result, totalpage: totalpage, page: page });
};
...
exports.reportCard = async (req, res) => {
  ...
  res.render("ReportCard", {
    ...
    name: namerlt,
    attendata: attenrlt,
    totalmark: totalmarkrlt,
    attenper: attenperrlt,
  });
};
